...
async function main() {
  ...
  // Query to get books with publisher and pubdate
  const query = `
    SELECT
      b.id,
      b.title,
  ...
  for (const book of calibreBooks) {
    ...
    const result = await pool.query(
      `SELECT id, publisher, publish_date FROM books
    ...
    // Skip if already has this metadata
    if (dbBook.publisher && dbBook.publish_date) {
      skipped++;
      continue;
    }
    ...
    await pool.query(
      `UPDATE books SET
         ...
         updated_at = NOW()
       WHERE id = $3`,
      [book.publisher, publishDate, dbBook.id]
    );
